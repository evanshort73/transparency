...
def getScaledImages(rgbScale, im1, im2):
    ...
    mean1 = np.mean(im1, axis = (0, 1))
    scaledIm1 = im1 - mean1
    scaledIm1 *= overallRgbScale[0]
    scaledIm1 += mean1
    
    mean2 = np.mean(im2, axis = (0, 1))
    scaledIm2 = im2 - mean2
    scaledIm2 *= overallRgbScale[1]
    scaledIm2 += mean2
...
# Use bogus calculations in order to match the image generated by the previous
# commit
adjustedColorSpaceCenter = -(0.5 * (np.mean(im1, axis = (0, 1)) \
                                    + np.mean(im2, axis = (0, 1))) \
                             - 0.5) \
                           * np.mean(rgbScale / initRgbScale(im1, im2),
                                     axis = 0)
...
def getTrueColors(adjustedIm1, adjustedIm2, hubCenter, idealAlpha,
    ...
    perChannelCutoff -= hubCenter + adjustmentToMatchPreviousCommit
    ...
    trueColors += hubCenter + adjustmentToMatchPreviousCommit
...
adjustmentToMatchPreviousCommit = -hubCenter - adjustedColorSpaceCenter + 0.5
trueColors = getTrueColors(adjustedIm1, adjustedIm2, hubCenter,
                           idealAlpha, adjustmentToMatchPreviousCommit)
